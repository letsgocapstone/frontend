...
export default function PlaceDetailPage() {
  ...
  <div className="max-w-2xl mx-auto px-4 py-8">
    <button onClick={handleGoBack} className="text-[#007bff] text-sm mb-4 hover:underline">
      ← 뒤로가기
    </button>
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <div className="p-5 border-b border-gray-100 space-y-2">
        ...
        <div className="flex gap-2 flex-wrap mt-2">
          {place.tags.map((tag) => (
            <span
              key={tag}
              className="border border-[#007bff] text-[#007bff] bg-blue-50 px-3 py-1 rounded-full text-sm"
            >
              #{tag}
            </span>
          ))}
      ...
      <div className="p-5 flex items-center gap-4 border-b border-gray-100">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-full text-white text-sm ${
            bookmarked ? 'bg-[#007bff]' : 'bg-gray-300 text-gray-700'
          }`}
        >
          <FaBookmark /> 저장됨
        </button>
